...
/**
 * Basitleştirilmiş TP (Take Profit) Price Testi
 * 
 * Bu test, TradingView ekran görüntülerindeki turkuaz/teal TP kutularını tespit eder
 * ve bu kutulardan fiyat değerlerini çıkarır.
 */

// TP için turkuaz/teal renk tanımları (görüntüden gözlemlenen)
const TP_COLORS = [
  { r: 0, g: 128, b: 128 },   // Turkuaz/teal
  { r: 0, g: 150, b: 136 },   // Açık turkuaz
  { r: 0, g: 130, b: 120 }    // Orta turkuaz
];

// Renk toleransı - TP ve current price renkleri arasında daha iyi ayrım için düşük değer
...
/**
 * Piksel renginin TP rengi olup olmadığını kontrol et
 */
function isTpColor(color: { r: number, g: number, b: number }): boolean {
  // Önce basit renk mesafesi kontrolü
  const isCloseToTpColor = TP_COLORS.some(tpColor => 
    colorDistance(color, tpColor) <= COLOR_TOLERANCE
  );
  
  // Eğer renkler yakın çıktıysa, daha hassas ayırt edici fonksiyonu kullan
  if (isCloseToTpColor) {
    return differentiateTPFromCurrentPrice(color) === 'tp';
  }
...
/**
 * Piksel renginin TP rengi mi yoksa current price rengi mi olduğunu daha hassas ayırt et
 * Turkuaz/teal (TP) ve yeşil (current price) tonları arasındaki fark bazen az olabilir
 */
function differentiateTPFromCurrentPrice(color: { r: number, g: number, b: number }): 'tp' | 'current' | 'other' {
  // TP renkleri için genellikle blue değeri green'e yakın veya daha yüksek olur
  if (color.b >= color.g * 0.7 && color.r < color.g * 0.5 && color.g > 100) {
    return 'tp'; // Turkuaz/teal (blue bileşeni yüksek)
  }
  
  // Current price yeşili için blue değeri daha düşük olur
  if (color.g > 100 && color.b < color.g * 0.5 && color.r < color.g * 0.5) {
    return 'current'; // Yeşil (green bileşeni dominant)
  }
  
  // Kırmızı current price
  if (color.r > 100 && color.g < color.r * 0.5 && color.b < color.r * 0.5) {
    return 'current'; // Kırmızı
  }
...
/**
 * TP kutularını tespit et ve değerlendir
 */
async function findTpBox(panelBuffer: Buffer): Promise<{ box: { x: number, y: number, width: number, height: number }, index: number } | null> {
  // Görüntüyü işle
  const { data, info } = await sharp(panelBuffer)
    .raw()
    .toBuffer({ resolveWithObject: true });
  
  // TP rengindeki pikselleri bul
  const tpPixels: { x: number, y: number }[] = [];
  ...
  console.log(`${tpPixels.length} adet turkuaz/teal TP pikseli bulundu.`);
  
  if (tpPixels.length === 0) {
    return null;
  }
  ...
  for (let y = 0; y < info.height; y++) {
    const pixelsInRow = tpPixels.filter(p => p.y === y).length;
  ...
  for (let i = 0; i < rowGroups.length; i++) {
    ...
    if (i > 0 && row.y - rowGroups[i-1].y <= 3) {
      ...
      if (currentBox) {
        ...
        currentBox.pixels.push(...tpPixels.filter(p => p.y === row.y));
      }
    } else {
      ...
      currentBox = {
        minY: row.y,
        maxY: row.y,
        pixels: tpPixels.filter(p => p.y === row.y)
      };
    }
  }
  ...
  console.log(`${filteredBoxes.length} adet potansiyel TP kutusu tespit edildi.`);
  
  if (filteredBoxes.length === 0) {
    return null;
  }
  
  // GELİŞTİRİLMİŞ TP KUTUSU SEÇİMİ:
  // TradingView'de TP kutuları için daha kapsamlı değerlendirme yapıyoruz
  
  // 1. Şekil ve Oran Puanlaması: TP kutularının genişlik/yükseklik oranı genellikle 3-5 arasındadır
  ...
  // 2. Konum Puanlaması: TP kutuları genellikle ekranın orta bölgesinde yer alır
  ...
  // 3. Boyut Puanlaması: TP kutuları genellikle ne çok büyük ne de çok küçük olur
  ...
  // 4. Doluluk Oranı Puanlaması: TP kutuları genellikle homojen turkuaz/teal renk dağılımına sahiptir
  const fillScores = filteredBoxes.map(box => {
    // Kutu içindeki turkuaz/teal piksel sayısını hesapla
    const boxPixels = tpPixels.filter(p => 
      p.x >= box.x && p.x < box.x + box.width && 
      p.y >= box.y && p.y < box.y + box.height
    );
    
    // Doluluk oranı: kutu içindeki turkuaz/teal pikseller / toplam kutu alanı
  ...
  // 5. Yatay Konum Puanlaması: TP kutuları genellikle panel ortasına yakın olur
  ...
  console.log(`En muhtemel TP kutusu seçildi (index: ${bestBoxIndex}, puan: ${totalScores[bestBoxIndex].toFixed(2)})`);
  console.log(`TP kutusu: x=${bestBox.x}, y=${bestBox.y}, genişlik=${bestBox.width}, yükseklik=${bestBox.height}`);
  
  return { box: bestBox, index: bestBoxIndex };
}

/**
 * Take Profit (TP) Price Testi
 * 
 * Bu test, TradingView ekran görüntülerindeki turkuaz/teal TP kutularını tespit eder
 * ve bu kutulardan fiyat değerlerini çıkarır.
 */
...
const CURRENT_PRICE_COLORS = [
  ...
  { r: 178, g: 34, b: 34 },  // Koyu kırmızı (düşüş)
  { r: 139, g: 0, b: 0 }     // Alternatif koyu kırmızı
];
...
function isCurrentPriceColor(color: { r: number, g: number, b: number }): boolean {
  ...
  if (isCloseToCurrentColor) {
    return differentiateTPFromCurrentPrice(color) === 'current';
  }
...
/**
 * TP kutusunun şekil ve boyutunun uygun olup olmadığını kontrol et
 * TP kutuları genellikle turkuaz/teal renkte olup current price'dan (yeşil veya kırmızı) farklıdır
 */
function isTPBoxShape(box: { width: number, height: number }): boolean {
  // Genişlik/yükseklik oranı kontrolü
  const aspectRatio = box.width / box.height;
  const isValidAspect = aspectRatio >= 3 && aspectRatio <= 5;
  
  // Boyut kontrolü
  const area = box.width * box.height;
  const isValidSize = area >= 150 && area <= 400;
  
  return isValidAspect && isValidSize;
}

/**
 * Tespit edilen TP kutularını görselleştir
 */
async function visualizeTpBoxes(
  panelBuffer: Buffer, 
  ...
  // TP box'larını işaretle - en iyi kutu turkuaz, diğerleri sarı
  ...
  const svg = `
    <svg width="${width}" height="${height}">
      ${allBoxes.map((box, index) => {
        ...
        const boxColor = isBest ? 'teal' : 'yellow';
        ...
        return `
          ...
          <text 
            ...
            fill="${boxColor}" 
            text-anchor="middle"
          >Box ${index + 1}${isBest ? ' (TP)' : ''}${scoreInfo}</text>
  ...
  fs.writeFileSync('tp_boxes_debug.png', markedImage);
  console.log('TP kutuları görselleştirildi: tp_boxes_debug.png');
}

/**
 * TP kutusundan metin çıkar
 */
async function extractTextFromTpBox(panelBuffer: Buffer, box: { x: number, y: number, width: number, height: number }): Promise<string | null> {
  try {
    ...
    fs.writeFileSync('tp_box_original.png', croppedBuffer);
    ...
    fs.writeFileSync('tp_box_processed_1.png', processedBuffer1);
    fs.writeFileSync('tp_box_processed_2.png', processedBuffer2);
    fs.writeFileSync('tp_box_processed_3.png', processedBuffer3);
    
    console.log('TP kutusu görüntüleri kaydedildi: tp_box_original.png, tp_box_processed_1-3.png');
    ...
    console.error('TP kutusundan metin çıkarma hatası:', error);
    return null;
  }
}

/**
 * TP değerlerini test et
 */
async function testTpPrice() {
  try {
    console.log('\n=== TP FİYAT TESPİT TESTİ BAŞLADI ===\n');
    ...
    // TP kutusunu tespit et - sadece en iyi kutuyu al
    console.log('\n2. TP kutusu tespit ediliyor...');
    const result = await findTpBox(panelBuffer);
    
    if (!result) {
      console.error('❌ TP kutusu tespit edilemedi!');
      return;
    }
    
    const { box: tpBox, index } = result;
    ...
    // TP kutularını görselleştir - puanları da geçir
    console.log('\n3. TP kutuları görselleştiriliyor...');
    await visualizeTpBoxes(panelBuffer, tpBox, filteredBoxes, scores);
    
    // Sadece TP kutusundan metni çıkar
    console.log('\n4. TP kutusundan metin çıkarılıyor...');
    const tpValue = await extractTextFromTpBox(panelBuffer, tpBox);
    
    // Sonuçları göster
    console.log('\n=== TP FİYAT SONUÇLARI ===');
    
    if (tpValue) {
      console.log(`\n✓ Tespit edilen TP fiyatı: ${tpValue}`);
    } else {
      console.error('❌ TP değeri çıkarılamadı!');
    }
